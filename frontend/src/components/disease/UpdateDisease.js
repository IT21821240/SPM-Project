...
import updateDiseaseImage from '../images/updateDiseaseImage.avif';
...
const UpdateDisease = () => {
  ...
  useEffect(() => {
    const fetchDisease = async () => {
      try {
        const { data } = await axios.get(`http://localhost:5000/api/diseases/${id}`, {
          headers: { Authorization: `Bearer ${auth.token}` }
        });

        const { name, symptoms, affectedSpecies, treatment, severity } = data.data;
        setName(name || '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/diseases/${id}`, {
        name,
  ...
  if (loading) {
    return (
      <div className="relative min-h-screen bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${updateDiseaseImage})` }}>
      ...
      <div className="flex items-center justify-center min-h-screen bg-black bg-opacity-60">
        <div className="flex max-w-4xl w-full">
          ...
          <div className="flex-1 p-8 bg-white bg-opacity-80 rounded-lg shadow-lg border border-gray-200">
            <h2 className="text-3xl font-bold text-gray-900 text-center mb-4">Update Disease</h2>
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-center text-white max-w-lg">
            <h1 className="text-5xl font-extrabold mb-4 leading-tight">Update Your Knowledge</h1>
            <p className="text-xl mb-4">
              Modify existing plant disease entries to ensure accurate and up-to-date information.
            </p>
          </div>
        </div>
      </div>
