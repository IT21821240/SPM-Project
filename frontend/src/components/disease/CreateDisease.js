...
import createDiseaseImage from '../images/addDiseaseImage.jpg';
...
const CreateDisease = () => {
  ...
  const validateSeverity = (severity) => ['Mild', 'Moderate', 'Severe'].includes(severity);
  ...
  const handleSubmit = async (e) => {
    ...
    const affectedSpeciesArray = affectedSpecies.split(',').map(s => s.trim()).filter(s => s);
  ...
  return (
    <div className="relative min-h-screen bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${createDiseaseImage})` }}>
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="flex max-w-4xl w-full">
            ...
            <div className="flex-1 p-8 bg-white bg-opacity-80 rounded-lg shadow-lg border border-gray-200">
              <h2 className="text-3xl font-bold text-gray-900 text-center mb-4">Add New Disease</h2>
              <p className="text-center text-gray-600 mb-6">Complete the form below to add a new disease entry.</p>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-4">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium text-gray-700">Disease Name</label>
                    <input
                      id="name"
                      name="name"
                      type="text"
                  ...
                  <div>
                    <label htmlFor="affectedSpecies" className="block text-sm font-medium text-gray-700">Affected Species</label>
                    <input
                      id="affectedSpecies"
                ...
                <div>
                  <button
                    type="submit"
                    className={`w-full py-3 px-4 border border-transparent text-sm font-medium rounded-md ${loading ? 'bg-indigo-500' : 'bg-indigo-600'} ${loading ? 'hover:bg-indigo-600' : 'hover:bg-indigo-700'} text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150 ease-in-out`}
                    disabled={loading}
                  >
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center text-white max-w-lg">
              <h1 className="text-5xl font-extrabold mb-4 leading-tight">Nurture Your Greenery</h1>
              <p className="text-xl mb-4">
                Share your knowledge by adding new plant diseases. Help others to better understand and manage their plants.
              </p>
            </div>
          </div>
        </div>
