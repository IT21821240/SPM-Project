...
const DiseaseList = () => {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto bg-white shadow-lg rounded-lg border border-gray-200">
      <h2 className="text-4xl font-bold text-gray-900 mb-8 text-center">Disease List</h2>
      {diseases.length > 0 ? (
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {diseases.map((disease) => (
            ...
            >
              <h3 className="text-2xl font-semibold text-gray-800">{disease.name}</h3>
              {disease.symptoms && (
              ...
              {disease.affectedSpecies && (
                <p className="text-gray-700 mt-2">
                  <strong>Affected Species:</strong> {disease.affectedSpecies.join(', ')}
                </p>
              )}
              {disease.treatment && (
                <p className="text-gray-700 mt-2">
              ...
              <p className={`mt-2 text-sm font-semibold ${disease.severity === 'Severe' ? 'text-red-600' : disease.severity === 'Moderate' ? 'text-yellow-600' : 'text-green-600'}`}>
